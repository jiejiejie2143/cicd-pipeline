...
# B="/dev/xvde"   #//亚马逊
B2="xvde"
A1=`lsblk |grep xvde|grep -v xvde1|awk '{print $1}'`	#//天翼云
if [[ $A1 == *$B2* ]]
then
    echo "包含/dev/xvde"
    if [ "`df -h | awk '{print $6}'|grep '^/data$'`" = "/data" ]
    then
      ...
      echo "/data未挂载,开始格式化磁盘"
	  [ ! -d /data ] && sudo mkdir -p /data
	  sudo parted /dev/xvde -s mklabel gpt
	  sudo parted /dev/xvde -s mkpart  primary 2048s 100%
	  sudo mkfs.ext4 -m0 /dev/xvde1
	  sudo bash -c 'echo "/dev/xvde1               /data                   ext4    defaults        0 0" >> /etc/fstab'
	  sudo mount -a
    fi
 else
    echo "不包含/dev/xvde"
