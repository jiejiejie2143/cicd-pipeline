...
def getParas(keyword,keyenv = env.appenv) {
    self_paras = sh  script: 'cat programs/' + env.project + '/'+keyenv+'_paras|grep ' + env.app_name + '_' + keyword + '|awk -F "=" \'{print $2}\''
    //self_paras = self_paras.tokenize('\n')[0]
    common_paras = sh  script: 'cat programs/' + env.project + '/'+keyenv+'_paras|grep ' + env.project + '_' + keyword + '|awk -F "=" \'{print $2}\''
